...
interface JijiSheet {
    ...
    weekly_puzzles: string,
    weekly_minutes: string,
    cumulative_minutes: string,
    cumulative_puzzles: string,
}
...
const getPuzzleClasses = (studentWeeks: JijiSheet[]): PuzzleClasses => {
    ...
    const classes = d3.nest()
        ...
        .rollup((cs:JijiSheet[]):PuzzleStudent=>{
            ...
            const totalProgress = parseInt(cs[0].cumulative_puzzles)/puzzlesByGrade[cs[0].syllabus_grade_student] * 100   //puzzlesByGrade[cs[0].syllabus_grade]
            const perPuzzleTime = parseInt(cs[0].cumulative_minutes) > 0 ? (parseInt(cs[0].cumulative_puzzles)/parseInt(cs[0].cumulative_minutes)).toFixed(1): '-'
            return {
                name: name,
                ...
                lastWeekCompleted: 100 * parseInt(lastWeek.weekly_puzzles)/puzzlesByGrade[cs[0].syllabus_grade_student],
                prevWeekCompleted: prevWeek !== 0 ? 100 * parseInt(prevWeek.weekly_puzzles)/puzzlesByGrade[cs[0].syllabus_grade_student] : prevWeek,
                lastWeekMinutes: parseInt(lastWeek.weekly_minutes),
                cumulativePuzzles: parseInt(lastWeek.cumulative_puzzles)
            }
        })
        .object(studentWeeks)
