...
import { createESGradebookReports } from "../gradebook-audit-backend";
import {
  GradeLogic,
  AssignmentImpact,
  TeacherClasses,
...
const defaultSummaries = {
  "Percent Students Failing Cutoff": true,
  "Unique Assignments Cutoff": true,
  "Percent Graded D or F Cutoff": true,
};

export class GradebookAuditReport extends React.PureComponent<
  GradebookAuditReportProps,
  GradebookAuditReportState
> {
  componentWillMount() {
    const teacherClasses = this.props.reportFiles
      ? createGradebookReports(this.props.reportFiles)
      : {};
    ...
    this.setState({
      ...
      sped:
        ...
        this.props.reportFiles.reportFiles[
          this.props.reportFiles.reportTitle.optionalFiles[0].fileDesc
        ]
          ? true
          : false,
  ...
  createAdminOverview(teacherClasses: TeacherClasses): AdminOverview {
    ...
    Object.keys(teacherClasses).forEach((teacher) => {
      Object.keys(teacherClasses[teacher]).forEach((className) => {
        if (
          teacherClasses[teacher][className].totalAsgn <
          this.state.uniqueAssignmentsCutoff
        ) {
          if (adminOverview.uniqueAssignmentFlag[teacher] == undefined) {
            adminOverview.uniqueAssignmentFlag[teacher] = {};
          }
          adminOverview.uniqueAssignmentFlag[teacher][className] =
            teacherClasses[teacher][className];
        }

        if (
          teacherClasses[teacher][className].pctDF >
          this.state.pctGradedDorFCutoff
        ) {
          if (adminOverview.pctGradedDFFlag[teacher] == undefined) {
            adminOverview.pctGradedDFFlag[teacher] = {};
          }
          adminOverview.pctGradedDFFlag[teacher][className] =
            teacherClasses[teacher][className];
        }

        if (
          teacherClasses[teacher][className].pctStudentsFailing >
          this.state.pctStudentsFailingCutoff
        ) {
          if (adminOverview.pctStudentFailingFlag[teacher] == undefined) {
            adminOverview.pctStudentFailingFlag[teacher] = {};
          }
          adminOverview.pctStudentFailingFlag[teacher][className] =
            teacherClasses[teacher][className];
        }
      });
    });
    ...
    return (
      <React.Fragment>
        <Modal
          ...
          onEntered={() => {
            this.generatePDFReports();
          }}
          onExited={() => {
            this.setState({ dlProgress: 0 });
          }}
        >
          <ModalHeader>Working...</ModalHeader>
          <ModalBody>
            Formatting {this.state.dlProgress}/
            {this.state.selectedTeachers.length === 0
        ...
        <Container>
          <Row>
            <Col className="gpa-filter-container">
              <Tabs>
                <Tab eventKey="teachers" title="Teachers">
                  ...
                  >
                    Generate Individual Report PDF's
                  </Button>
                ...
                <Tab eventKey="summary" title="Summary">
                  <Form>
                    ...
                    <AdminSlider
                      ...
                      initial={10}
                      checked={
                        this.state.visibleSummaries[
                          "Percent Students Failing Cutoff"
                        ]
                      }
                      handleSlider={(n) =>
                        this.setState({ pctStudentsFailingCutoff: n })
                      }
                    ...
                    <AdminSlider
                      ...
                      initial={7}
                      checked={
                        this.state.visibleSummaries["Unique Assignments Cutoff"]
                      }
                      handleSlider={(n) =>
                        this.setState({ uniqueAssignmentsCutoff: n })
                      }
                      handleCheck={this.handleAdminCheck}
                    />
                    <AdminSlider
                      ...
                      checked={
                        this.state.visibleSummaries[
                          "Percent Graded D or F Cutoff"
                        ]
                      }
                      handleSlider={(n) =>
                        this.setState({ pctGradedDorFCutoff: n })
                      }
                      handleCheck={this.handleAdminCheck}
                    />
                  </Form>
            ...
            <Col
              className="gpa-display-container"
              id="gpa-display-container"
              onScroll={(ev) => {
                this.setState({
                  auditScroll: document.getElementById("gpa-display-container")
                    ?.scrollTop as number,
                });
              }}
            >
              {this.state.selectedTeachers.length === 0 ? (
                <AdminOverviewSheet
                  ...
                  adminOverview={this.createAdminOverview(
                    this.state.teacherClasses
                  )}
              ...
              <React.Fragment>
                {teachers.map((teacher) => {
                  ...
                  if (teacherClasses[tKey]) {
                    ...
                    const [hasGrades, noGrades]: string[][] = partition(
                      (cn: string) => {
                        if (
                          teacherClasses[tKey][cn].distribution === undefined
                        ) {
                          console.log(teacherClasses[tKey]);
                          console.log(cn);
                        }
                        return teacherClasses[tKey][cn].hasGrades;
                      },
                      Object.keys(teacherClasses[tKey])
                    );
                    //hasAsgn list of classes with assignments
                    const hasAsgn: string[] = Object.keys(
                      teacherClasses[tKey]
                    ).filter((cn) => {
                    ...
                    return (
                      <div
                        key={tKey}
                        className="gradebook-audit-report"
                        id={tKey}
                      >
                        <h1>{tKey}</h1>
                        <GradeDistributionDisplay
  ...
  handleTeacherClick = (staff: string[] | string): void => {
    ...
    if (Array.isArray(staff)) {
      if (staff.every((s) => selected.includes(s))) {
        this.setState({
          selectedTeachers: selected.filter((f) => !staff.includes(f)),
        });
      } else {
        const newSelected = selected.concat(
          staff.filter((s) => !selected.includes(s))
        );
      ...
      selected.includes(staff)
        ? this.setState({
            selectedTeachers: selected.filter((f) => f !== staff),
          })
...
const AdminSlider: React.FunctionComponent<{
  label: string;
  min: number;
  max: number;
  initial: number;
  checked: boolean;
  handleSlider: (number) => void;
  handleCheck: (string) => void;
}> = (props) => {
...
const TopButton: React.FunctionComponent<{
  scroll: number;
  backToTop: (ev: Event) => void;
}> = (props) => {
  return (
    <Button
      onClick={props.backToTop as any}
      id="btn-back-to-top"
      style={{
        position: "fixed",
        bottom: "20px",
